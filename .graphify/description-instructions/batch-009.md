...
- "tests_test_api_testcsrfguard": "TestCsrfGuard" | kind=code-symbol | source=tests/test_api.py:L176 | neighbors=[test_api.py, .test_delete_without_xrw_header_rejecte…, .test_get_requests_not_blocked(), .test_post_without_json_content_type_re…, .test_valid_delete_request_allowed(), .test_post_without_xrw_header_rejected()] | lang=en
- "vendor_chart_4_5_1_umd_min_bo_buildticks": ".buildTicks()" | kind=code-symbol | source=vendor/chart-4.5.1.umd.min.js:L13 | neighbors=[bo, .getTickLimit(), H(), j(), s(), ._maxDigits()] | lang=en
