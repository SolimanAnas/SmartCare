...
def main():
    ...
    for folder in course_folders:
        ...
        for ch in chapters:
            ...
            # Since only chapter 1 is implemented currently, skip missing warnings for other chapters
            ...
            try:
                import subprocess
                # Run node to evaluate the JS and dump the window.CPG_DATA object as clean JSON
                normalized_path = ch_filepath.replace("\\", "/")
                node_code = f"global.window = {{}}; require('{normalized_path}'); console.log(JSON.stringify(global.window.CPG_DATA));"
                result = subprocess.run(["node", "-e", node_code], capture_output=True, text=True)
                
                if result.returncode != 0:
                    print(f"    [-] ERROR: Node.js failed to execute {ch_filename}: {result.stderr}")
                    has_errors = True
                    continue
                ...
                ch_required = ["id", "title", "shortTitle", "sections", "flashcards", "quiz", "critical"]
                ...
                print(f"      [+] Verified {len(sections)} sections, {len(flashcards)} flashcards, {len(quizzes)} quiz questions, and {len(criticals)} critical scenarios.")
                
                # Assert counts for Chapter 1
                if ch_id == "c01":
                    if len(flashcards) < 20:
                        print(f"      [-] WARNING: Flashcards count is {len(flashcards)}, expected at least 20.")
                    if len(quizzes) < 20:
                        print(f"      [-] WARNING: Quiz question count is {len(quizzes)}, expected at least 20.")
                    if len(criticals) < 5:
                        print(f"      [-] WARNING: Scenario count is {len(criticals)}, expected at least 5.")
