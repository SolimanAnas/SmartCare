...
// Smoke test for the BDLS course reviewer (courses/bdls/index.html). Verifies
// the app boots from data/bundle.js (window.BDLS_DATA), renders the chapter
// sidebar and all six content tabs, grades a quiz answer, switches chapters,
// and stays free of horizontal overflow on phones. Modeled on the BLS spec.

test('BDLS reviewer boots and renders sidebar + first chapter', async ({ page }) => {
  await page.goto('courses/bdls/index.html', { waitUntil: 'networkidle' });

  // Bundle loaded the dataset onto the global.
  const hasData = await page.evaluate(() => !!(window.BDLS_DATA && window.BDLS_DATA.manifest));
  ...
  // First chapter title renders in the header (PRE-DISASTER Paradigm).
  await expect(page.locator('#ctitle')).toContainText('PRE-DISASTER');
});

test('BDLS reviewer exposes all six content tabs', async ({ page }) => {
  await page.goto('courses/bdls/index.html', { waitUntil: 'networkidle' });
...
test('BDLS reviewer quiz tab renders questions and grades an answer', async ({ page }) => {
  await page.goto('courses/bdls/index.html', { waitUntil: 'networkidle' });
...
test('BDLS reviewer switches chapters via the sidebar', async ({ page }) => {
  await page.goto('courses/bdls/index.html', { waitUntil: 'networkidle' });
  // Click the SALT triage chapter and confirm the header updates.
  await page.locator('#chapters .chap', { hasText: 'SALT' }).click();
  await expect(page.locator('#ctitle')).toContainText('SALT');
});
...
for (const vp of [{ w: 360, h: 740 }, { w: 320, h: 568 }]) {
  test(`BDLS reviewer has no horizontal overflow at ${vp.w}px`, async ({ page }) => {
    await page.setViewportSize({ width: vp.w, height: vp.h });
    await page.goto('courses/bdls/index.html', { waitUntil: 'networkidle' });
    await page.locator('#chapters .chap', { hasText: 'SALT' }).first().click();
