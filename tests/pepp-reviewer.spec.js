...
// Smoke test for the PEPP course reviewer (courses/pepp/index.html).
// Verifies the app boots from data/bundle.js (window.PEPP_DATA), renders
...
// Modeled on the BLS/BDLS/EMPACT specs.

test('PEPP reviewer boots and renders sidebar + first chapter', async ({ page }) => {
  await page.goto('courses/pepp/index.html', { waitUntil: 'networkidle' });

  // Bundle loaded the dataset onto the global.
  const hasData = await page.evaluate(() => !!(window.PEPP_DATA && window.PEPP_DATA.manifest));
  ...
  await expect(page.locator('#ctitle')).toContainText('Why Children Are Different');
});

test('PEPP reviewer exposes all six content tabs', async ({ page }) => {
  await page.goto('courses/pepp/index.html', { waitUntil: 'networkidle' });
...
test('PEPP reviewer quiz tab renders questions and grades an answer', async ({ page }) => {
  await page.goto('courses/pepp/index.html', { waitUntil: 'networkidle' });
...
test('PEPP reviewer switches chapters via the sidebar', async ({ page }) => {
  await page.goto('courses/pepp/index.html', { waitUntil: 'networkidle' });
  // Click the Pediatric Assessment Triangle chapter and confirm the header updates.
  await page.locator('#chapters .chap', { hasText: 'Pediatric Assessment Triangle' }).click();
  await expect(page.locator('#ctitle')).toContainText('Pediatric Assessment Triangle');
});
...
// (EMS Operations / JumpSTART triage tables) at phone widths and fails on any
// page-level horizontal overflow.
for (const vp of [{ w: 360, h: 740 }, { w: 320, h: 568 }]) {
  test(`PEPP reviewer has no horizontal overflow at ${vp.w}px`, async ({ page }) => {
    await page.setViewportSize({ width: vp.w, height: vp.h });
    await page.goto('courses/pepp/index.html', { waitUntil: 'networkidle' });
    await page.locator('#chapters .chap', { hasText: 'MCI Triage' }).first().click();
